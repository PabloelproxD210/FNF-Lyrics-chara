...
class PlayState extends MusicBeatState
{
	...
	//momazos chara XDD
	var hasLyricsbf:Bool = false;
	var curLyrStepbf:String = '';
	var lyrTextbf:String = '';
	var lyrAddedbf:Bool = false;
	var lyrObjbf:FlxText;
	var lyricStepsbf:Array<String>;

	var hasLyricsdad:Bool = false;
	var curLyrStepdad:String = '';
	var lyrTextdad:String = '';
	var lyrAddeddad:Bool = false;
	var lyrObjdad:FlxText;
	var lyricStepsdad:Array<String>;
	...
	override public function create()
	{
		...
		//no se por que me marca error pero igual no importa porque sigue dejando exportar XD
		hasLyricsbf = FileSystem.exists(Paths.lyric(PlayState.SONG.song.toLowerCase()  + "/lyricsbf"));
		trace('Lyric File: ' + hasLyricsbf + " - " + Paths.lyric(PlayState.SONG.song.toLowerCase() + "/lyricsbf"));

		lyricStepsbf = null;
		if (hasLyricsbf)
		{
			lyricStepsbf = CoolUtil.coolTextFile(Paths.lyric(PlayState.SONG.song.toLowerCase() + "/lyricsbf"));
			var splitStep:Array<String> = lyricStepsbf[0].split(":");
			curLyrStepbf = splitStep[1];
			lyrTextbf = lyricStepsbf[0].substr(splitStep[1].length + 2).trim();
		}

		hasLyricsdad = FileSystem.exists(Paths.lyric(PlayState.SONG.song.toLowerCase()  + "/lyricsdad"));
		trace('Lyric File: ' + hasLyricsbf + " - " + Paths.lyric(PlayState.SONG.song.toLowerCase() + "/lyricsdad"));

		lyricStepsdad = null;
		if (hasLyricsdad)
		{
			lyricStepsdad = CoolUtil.coolTextFile(Paths.lyric(PlayState.SONG.song.toLowerCase() + "/lyricsdad"));
			var splitStep:Array<String> = lyricStepsdad[0].split(":");
			curLyrStepdad = splitStep[1];
			lyrTextdad = lyricStepsdad[0].substr(splitStep[1].length + 2).trim();
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'screwed':
				{
					defaultCamZoom = 0.85;
					curStage = 'screwed';
					var bg:FlxSprite = new FlxSprite(-350, -270).loadGraphic(Paths.image('bambi/farmland'));
					bg.antialiasing = FlxG.save.data.antialiasing;
					bg.setGraphicSize(Std.int(bg.width * 1.251111));
					bg.updateHitbox();
		...
		//bf
		lyrObjbf = new FlxText(0, 0, 0, "", 30);
		lyrObjbf.setFormat(Paths.font("chara.otf"), 50, FlxColor.fromString('#' + boyfriend.iconColor), CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		lyrObjbf.scrollFactor.set();
		lyrObjbf.alignment = CENTER;
		//lyrObjbf.screenCenter(X);
		lyrObjbf.x += 380;
		lyrObjbf.y += 480; //esta
		lyrObjbf.updateHitbox();
		add(lyrObjbf);
		lyrObjbf.text = '';


		//dad
		lyrObjdad = new FlxText(0, 0, 0, "", 30);
		lyrObjdad.setFormat(Paths.font("chara.otf"), 50, FlxColor.fromString('#' + dad.iconColor), CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		lyrObjdad.scrollFactor.set();
		lyrObjdad.alignment = CENTER;
		//lyrObjdad.screenCenter(X);
		lyrObjdad.y += 380; //esta
		lyrObjdad.x += 380;
		//lyrObj.y += 200;
		lyrObjdad.updateHitbox();
		add(lyrObjdad);
		lyrObjdad.text = '';
		...
		lyrObjbf.cameras = [camNotes];
		lyrObjdad.cameras = [camNotes];
		if (loadRep)
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (exDad) dad2.playAnim('sing' + sDir[daNote.noteData] + altAnim, true);
	...
	*/
	...
	override function stepHit()
	{
		...
		if ((("" +curStep) == curLyrStepbf) && hasLyricsbf)
			{
				if (lyrAddedbf = false)
				{
					lyrAddedbf = true;
				}
				lyrObjbf.text = lyrTextbf;
				trace(lyrTextbf);
				lyricStepsbf.remove(lyricStepsbf[0]);
				var splitStep:Array<String> = lyricStepsbf[0].split(":");
				curLyrStepbf = splitStep[1];
				lyrTextbf = lyricStepsbf[0].substr(splitStep[1].length + 2).trim();
			}

			if ((("" +curStep) == curLyrStepdad) && hasLyricsdad)
			{
				if (lyrAddeddad = false)
				{
					lyrAddeddad = true;
				}
				lyrObjdad.text = lyrTextdad;
				trace(lyrTextdad);
				lyricStepsdad.remove(lyricStepsdad[0]);
				var splitStep:Array<String> = lyricStepsdad[0].split(":");
				curLyrStepdad = splitStep[1];
				lyrTextdad = lyricStepsdad[0].substr(splitStep[1].length + 2).trim();
			}
